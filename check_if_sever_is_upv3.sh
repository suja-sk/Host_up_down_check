...
# Prompt user to enter server name and port as the input
read -p " Enter the name of the file containing server name and port number : " server_info_file

# Check that file is exits
...
do
    ...
    if [ $? -eq 0 ]
    then
        echo "Server $server_name $server_port is up and running"
    else
        echo "There is an error checking server $server_name $server_port "
    fi
